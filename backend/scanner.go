...
func FastAuditProject(ctx context.Context, p Project) (DashboardMetrics, error) {
	var metrics DashboardMetrics

	// 1. Fast FIM Metrics from SQLite
	rows, err := db.QueryContext(ctx, "SELECT status, COUNT(*) FROM project_files WHERE project_id=? GROUP BY status", p.ID)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var status string
			...
			if err := rows.Scan(&status, &count); err == nil {
				switch status {
				case "ADDED":
					metrics.NewFilesCount = count
				case "MODIFIED":
					metrics.ModifiedFilesCount = count
				case "DELETED":
					metrics.DeletedFilesCount = count
				case "ORPHAN":
					metrics.OrphanFilesCount = count
				}
			}
		}
	} else {
...
func reconcileOJSFiles(ctx context.Context, p Project, added []ProjectFile, modified []ProjectFile) ([]ProjectFile, error) {
...
func queryDBMetrics(ctx context.Context, p Project) (DashboardMetrics, error) {
...
func getOJSDetails(ctx context.Context, p Project) (OJSDetails, error) {
	var d OJSDetails
